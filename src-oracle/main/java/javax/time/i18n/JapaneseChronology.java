...
//    }
    ...
    private static final class EraRule extends DateTimeRule implements Serializable {
        ...
        /** Constructor. */
        private EraRule() {
            super("JapaneseEra", periodEras(), null, -3, 2, null);
        }
        private Object readResolve() {
            return INSTANCE;
        }
        ...
        protected void normalize(CalendricalEngine engine) {
            DateTimeField yoeVal = engine.getFieldDerived(JapaneseChronology.yearOfEraRule(), false);
            if (yoeVal != null) {
                // era, year, month, day-of-month
                JapaneseEra era = JapaneseEra.of(engine.getFieldDerived(this, false).getValidIntValue());
                DateTimeField moyVal = engine.getFieldDerived(JapaneseChronology.monthOfYearRule(), false);
                DateTimeField domVal = engine.getFieldDerived(JapaneseChronology.dayOfMonthRule(), false);
                if (moyVal != null && domVal != null) {
...
//                    engine.removeProcessed(JapaneseChronology.yearOfEraRule());
//                    engine.removeProcessed(JapaneseChronology.monthOfYearRule());
//                    engine.removeProcessed(JapaneseChronology.dayOfMonthRule());
                }
                // era, year, day-of-year
                DateTimeField doyVal = engine.getFieldDerived(JapaneseChronology.dayOfYearRule(), false);
                if (doyVal != null) {
                    JapaneseDate date = JapaneseDate.of(era, yoeVal.getValidIntValue(), MonthOfYear.JANUARY, 1).plusDays(doyVal.getValidIntValue() - 1);
                    engine.setDate(date.toLocalDate(), true);
//                    engine.removeProcessed(this);
//                    engine.removeProcessed(JapaneseChronology.yearOfEraRule());
//                    engine.removeProcessed(JapaneseChronology.dayOfYearRule());
                }
            }
        }
        @Override
        protected DateTimeField deriveFrom(CalendricalEngine engine) {
            JapaneseDate date = engine.derive(JapaneseDate.rule());
    ...
    private static final class YearOfEraRule extends DateTimeRule implements Serializable {
        ...
        private YearOfEraRule() {
            super("JapaneseYearOfEra", periodYears(), periodEras(),
                    JapaneseDate.MIN_YEAR_OF_ERA, JapaneseDate.MAX_YEAR_OF_ERA, null);
        }
        private Object readResolve() {
            return INSTANCE;
        }
    ...
    private static final class MonthOfYearRule extends DateTimeRule implements Serializable {
        ...
        private MonthOfYearRule() {
            super("JapaneseMonthOfYear", periodMonths(), periodYears(), 1, 12, null);
        }
        private Object readResolve() {
            return INSTANCE;
        }
    ...
    private static final class DayOfMonthRule extends DateTimeRule implements Serializable {
        ...
        private DayOfMonthRule() {
            super("JapaneseDayOfMonth", periodDays(), periodMonths(), DateTimeRuleRange.of(1, 28, 31), null);
        }
        private Object readResolve() {
            return INSTANCE;
        }
    ...
    private static final class DayOfYearRule extends DateTimeRule implements Serializable {
        ...
        private DayOfYearRule() {
            super("JapaneseDayOfYear", periodDays(), periodYears(), DateTimeRuleRange.of(1, 365, 366), null);
        }
        private Object readResolve() {
            return INSTANCE;
        }
    ...
    private static final class DayOfWeekRule extends DateTimeRule implements Serializable {
        ...
        private DayOfWeekRule() {
            super("JapaneseDayOfWeek", periodDays(), periodWeeks(), 1, 7, null);
        }
        private Object readResolve() {
            return INSTANCE;
        }
